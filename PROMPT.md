...
Slide Reference Placement and Format (FIRST LINE, ZERO-PADDED, CRITICAL FOR SORTING):
 ...
 - Write it as an opening square bracket, then the slide number padded with leading zeros to EXACTLY three digits, then an optional sequence letter described below, then the optional |USMLE label described below, then a closing square bracket: [003], [017], [042], [115], [099a], [042|USMLE].
...
Within-Slide Sequence Letter (REQUIRED WHENEVER ONE SLIDE PRODUCES MORE THAN ONE CARD):
 ...
 - So when a single slide produces two or more cards, give EVERY card from that slide a single lowercase letter placed straight after the number, immediately before the closing bracket or before the |USMLE label where the card carries one, assigned in the order the content appears on the slide, top to bottom and left to right: [099a], [099b], [099c].
 ...
 - A range or joined key carries its letter in the same position, straight after its last number: [100-099a], [100-099b], [046+063a], [100-099b|USMLE].
...
USMLE Label (INSIDE THE KEY, ONLY WHERE THE BACK CARRIES A [USMLE] PARAGRAPH):
 - A card whose Extra field carries a [USMLE] paragraph (see THE USMLE LAYER under EXTRA) also carries the label |USMLE inside its key, written as the LAST thing before the closing bracket: [002|USMLE]. The label and the paragraph always travel together. A labelled key with no [USMLE] paragraph behind it, or a [USMLE] paragraph under an unlabelled key, is wrong.
 - The label follows everything else in the key, including any range, joined number or sequence letter: [099b|USMLE], [013-012|USMLE], [046+063a|USMLE].
 - WRITE IT WITH A VERTICAL BAR, NEVER WITH A COMMA OR A SPACE. Anki sorts the Sort Field as plain text, one character at a time, so the character straight after the digits decides where a key lands. A comma sorts BELOW the hyphen: [013, USMLE] would jump ahead of [013-012], and the card covering slides 12 and 13 would fall after slide 13 instead of before it. A space sorts below everything and does the same damage. The vertical bar sorts above the hyphen, the plus sign and the closing bracket, so a labelled key lands exactly where the same key without the label would: [012], [013-012], [013|USMLE], [014].
 - Write it exactly |USMLE: an ASCII vertical bar, the word in capitals, no space on either side, and nothing else added inside the brackets.
 - The label changes nothing else. The card still files under the same slide, keeps the same sequence letter and shows the same image: [013|USMLE] still ends with <img src='xx-013.jpg'>.
...
Verify every line and fix any line that fails before printing the code block. Run the mechanical checks with a script rather than by eye, because a single stray quote silently corrupts an import:
 - Every line begins with a double quote, then an opening square bracket, then exactly three digits, then optionally a - or + joiner with three more digits, then optionally one lowercase sequence letter, then optionally the label |USMLE, then a closing bracket. No comma and no space appears anywhere inside a key.
 ...
 - A key carries the |USMLE label if and only if the Extra field on that same line holds a [USMLE] paragraph. Check both directions with a script, not by eye.
 ...
 - The annotation lines appear at most once each, in the order m.) then x.), after the citation and before any [USMLE] or [HOUSE] paragraph and the image, separated from each other by a single <br> and from the citation by <br><br>.
 - Where they appear, the [USMLE] paragraph comes before the [HOUSE] paragraph, each appears at most once, each opens its own block after <br><br>, and both come before the image. The markers are written exactly [USMLE] and [HOUSE, ssN, epN] with unpadded numbers, and they are the only square brackets in any Extra field.
 - No [USMLE] paragraph claims that a fact appeared on a particular exam, year, form or question bank, cites a review-book page or edition, gives a drug dose, carries a mnemonic, or restates the front.
 - Every [HOUSE] paragraph names an episode whose season, number, title and diagnosis you checked against the diagnosis list, that diagnosis is the disease the card teaches, and no episode is used on more than one card. Name every episode you used in your response text and say whether you checked the list.
...
Two short annotation lines may follow the citation. Each is optional, each appears at most once on a card, and where both are present m.) always comes first. They sit AFTER the citation and BEFORE any [USMLE] or [HOUSE] paragraph and the slide image, separated from the citation by <br><br> and from each other by a SINGLE <br>, because together they form one compact annotation block rather than two separate paragraphs.
...
THE USMLE LAYER ([USMLE] PARAGRAPH PLUS THE |USMLE KEY LABEL — OPTIONAL, AT MOST ONE PER CARD)
The deck is built for a six-year medical programme, but the reader also plans to sit the USMLE after graduating. Where a card's content is genuinely board material, the card says so twice: the key on the front carries the |USMLE label (see USMLE Label under the Text field rules), and the Extra field carries a paragraph opening with [USMLE] that gives the board's angle on that same content.
 - ADD IT ONLY WHERE THE CARD'S OWN CONTENT IS BOARD MATERIAL: the fact the card tests, or the disease or mechanism it directly explains, is a recognised high-yield USMLE topic, meaning First Aid for the USMLE Step 1 covers it or it is a standard theme of Step 1 or Step 2 CK review. A named disease with its enzyme or gene, a classic presentation, a pathognomonic finding, a drug's mechanism or adverse effect, or the anatomy behind a classic lesion usually qualifies. The history of a discovery, a lecturer's own classification, bench technique or a local protocol does not.
 - DO NOT ADD IT BY DEFAULT. A card that merely touches a board subject does not qualify, and neither does one where all you could write is the front again. A deck in which nearly every key carries the label has stopped telling the reader anything. A clinical lecture will earn more labels than a basic-science one, but never all of them.
 - WHAT IT SAYS: the angle the exam takes, which the lecture usually does not give. Pick what is true and useful for this card, usually one or two of: the classic vignette and its buzzwords, the classic association or distractor, the finding or lab that clinches it, first-line management at the level First Aid states it, or the mechanism-to-disease link a question stem is built on.
 - FORMAT: the marker [USMLE] in capitals, one space, then plain prose: [USMLE] Classic vignette: ... It runs one to three sentences and about 60 words at most. It points at the board angle. It is not a review-book page.
 - IT MUST ADD SOMETHING. Never restate the front, the context line or the x.) line.
 - THE ACCURACY RULES APPLY IN FULL. Only settled fact that First Aid and the standard review sources state the same way. No drug doses, no figures you are not sure of, no guideline-year specifics.
 - NEVER CLAIM PROVENANCE YOU CANNOT SHOW. Never say a fact appeared on a particular exam, year, form or question bank, never quote a First Aid page or edition, and never cite a review book unless it was actually attached. Real USMLE items are confidential. The label means this is board material, never this was on the exam.
 - WRITE IT IN YOUR OWN WORDS. Never copy a review book's sentences or tables.
 - A mnemonic still belongs on the m.) line, under that line's rules, and never inside this paragraph.
 - The enzyme and process markup, the arrow rule and the MathJax rule apply here exactly as in the Text field. The paragraph never contains a cloze.
 - THE PARAGRAPH DECIDES THE LABEL. If you cannot write a [USMLE] paragraph that meets these rules, the key stays unlabelled.

THE HOUSE M.D. LAYER ([HOUSE, ssN, epN] PARAGRAPH — OPTIONAL, AT MOST ONE PER CARD)
The reader is a House M.D. fan, and a case they have watched unfold is a far stronger hook than a list of features. Where the disease a card teaches was the diagnosis in an episode, the Extra field carries a short paragraph retelling that case.
 - ADD IT ONLY WHERE THE DIAGNOSIS MATCHES: the disease the card is about, or the disease its mechanism directly produces, must be a diagnosis listed for that episode in the List of medical diagnoses on the House wiki: https://house.fandom.com/wiki/List_of_medical_diagnoses. A disease named only on the x.) line or in passing does not qualify, and neither does a symptom or a test that merely shares a word with a diagnosis.
 - CHECK THE LIST, DO NOT RECALL IT. If you can fetch web pages, read the list ONCE before authoring and match every card against it. Its season tables give the episode number, the title, the patient and the diagnosis. If the page refuses your fetch, the same table is served as wikitext at https://house.fandom.com/api.php?action=parse&page=List_of_medical_diagnoses&prop=wikitext&format=json, and each episode's plot recap at the same address with page= set to the URL-encoded episode title, spaces written as underscores. Read the recap of every episode you use before writing its paragraph.
 - IF YOU CANNOT REACH THE LIST, write a House paragraph only for an episode whose season, number, title and diagnosis you are certain of, and say in your response text that the list was not checked. A wrong episode is exactly the confident kind of error this prompt exists to prevent.
 - FORMAT: the marker [HOUSE, ssN, epN], giving the season and the episode's number within that season, both unpadded, then one space, the episode title, a colon, and the prose: [HOUSE, ss1, ep22] Honeymoon: ... Write the marker exactly in that form, in capitals. Write an ampersand in a title as and, because a bare ampersand in an Anki field can turn into an HTML entity.
 - WHAT IT SAYS, IN THIS ORDER: who the patient is and how they present, the wrong turns (the other diagnoses the team chased and the tests they ran, with what those showed), the twist, and the clue or test that finally cracks the case. Name the other diseases and the tests, not only the final answer, because the differential is half the value. A memorable side detail, such as an unusual test or a treatment gamble, may go in if you are sure of it.
 - LENGTH: three to five sentences, about 120 words at most. It is a recap, not a transcript.
 - RETELL THE EPISODE, NEVER TEACH FROM IT. The show dramatises medicine. Report what happened as the episode's events, never as textbook fact. Where a clue in the episode contradicts textbook medicine, add a short note in round brackets saying so, as the porphyria example below does. The paragraph never carries a fact the front depends on.
 - NEVER INVENT a plot point, a test, a result, a patient name or a line of dialogue. Fewer details that are right beat more that are guessed. If you know the episode but not its plot, write only what you know or leave the paragraph off.
 - ONE EPISODE PER CARD, AND EACH EPISODE ON ONE CARD ONLY. If several episodes share the diagnosis, choose the one where it is the main patient's final diagnosis and whose case best fits the card. If several cards teach the same disease, the episode goes on the card that teaches the diagnosis most directly, never on all of them.
 - Spoilers are expected. The paragraph exists to bring the case back.
...
"[038|USMLE] Mineral metabolism:<br><b>Neuromuscular Signs of Low Calcium</b><br><br><b>Why Low Calcium Excites Nerves</b><br><br>- Low extracellular calcium lowers the threshold for {{c1::<u>sodium channel</u>}} opening, so nerve and muscle fire spontaneously.<br><br><b>Bedside Signs</b><br><br>- Facial twitching when the facial nerve is tapped in front of the ear is {{c2::<u>Chvostek</u>}} sign.<br><br>- Carpal spasm when a blood pressure cuff is held above systolic pressure is {{c2::<u>Trousseau</u>}} sign.<br>";"<font color='#55aaff'>Low magnesium has to be corrected first, because the parathyroid glands need magnesium to secrete PTH. — chuaypen2025<br><br>[USMLE] Classic vignette: perioral numbness, tingling fingers and muscle cramps shortly after thyroidectomy, from damage to the parathyroid glands. Expect a prolonged QT interval on the ECG.<br><br><img src='mine-038.jpg'></font>";"dxhypocalcemia"

"[034|USMLE] Amino acid metabolism:<br><b>Acute Intermittent Porphyria</b><br><br><b>The Enzyme Block</b><br><br>- It is a deficiency of {{c1::<u>⟨porphobilinogen deaminase⟩</u>}} in {heme synthesis}.<br><br>- The precursors upstream of the block, delta-aminolevulinic acid and {{c1::<u>porphobilinogen</u>}}, accumulate.<br><br><b>What Precipitates an Attack</b><br><br>- Drugs that induce ⟨cytochrome P450⟩ raise heme demand --> {{c2::<u>⟨ALA synthase⟩</u>}} is induced --> more precursor piles up behind the block.<br><br>- Alcohol and {{c2::<u>fasting</u>}} precipitate attacks as well.<br>";"<font color='#55aaff'>Most carriers never have an attack, because their remaining enzyme activity covers basal heme demand until something drives ⟨ALA synthase⟩ upward. — chuaypen2025<br><br>x.) not porphyria cutanea tarda, which blisters sun-exposed skin — acute intermittent porphyria causes no photosensitivity, because the precursors it accumulates are not light-activated<br><br>[USMLE] Classic vignette: acute abdominal pain, peripheral neuropathy, psychiatric symptoms and port-wine urine after a precipitating trigger. Treatment is hemin plus glucose, both of which suppress ⟨ALA synthase⟩.<br><br>[HOUSE, ss1, ep22] Honeymoon: Mark Warner, husband of House's ex-girlfriend Stacy, has abdominal pain and mood swings his doctors blamed on stress. Ultrasound and CT are clean and exploratory surgery finds only a distended bladder, until the surgery video shows abdominal epilepsy mistaken for peristalsis. Encephalitis and Alzheimer disease are ruled out, and new numbness in his toes and hands is treated as Guillain-Barré syndrome despite negative tests. The twist: he describes a Paris honeymoon that never happened and has started avoiding sunlight, so House suspects porphyria. The urine test only works mid-attack, so House triggers one against Mark's refusal and draws urine straight from the bladder to confirm it. (Real AIP causes no photosensitivity, so that clue is the show's.)<br><br><img src='aam-034.jpg'></font>";"bsxporphyria"
